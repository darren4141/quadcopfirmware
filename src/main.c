...
static esp_err_t mpu_basic_init(void) {
    ...
    //Sample rate divider
    ESP_ERROR_CHECK(i2c_write_reg(mpu_addr, MPU_REG_SMPLRT_DIV, 19));
    //Gyro config
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            for (uint16_t i = 0; i < n_to_read; i += DMP_PACKET_SIZE) {
                ...
                if (i + DMP_PACKET_SIZE >= n_to_read) {
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "MPU6050 detected at 0x%02X", mpu_addr);
    ESP_ERROR_CHECK(mpu_basic_init());

    // DMP init
    ESP_ERROR_CHECK(mpu_dmp_initialize());
    ESP_LOGI(TAG, "DMP ready; packet size = %d", DMP_PACKET_SIZE);
