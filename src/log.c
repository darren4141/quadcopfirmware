...
char log_name[MAX_NUM_ELEMENTS][NAME_LENGTH];
float log_val[MAX_NUM_ELEMENTS][MAX_NUM_VALS];
size_t log_num_vals[MAX_NUM_ELEMENTS];
...
void inline log_lock(){
    if(log_mutex != NULL){
        xSemaphoreTake(log_mutex, portMAX_DELAY);
    }
}

void inline log_unlock(){
    if(log_mutex != NULL){
    ...
    while(1){
        ...
        for(int i = 0; i < MAX_NUM_ELEMENTS; i++){
            ...
            for(size_t j = 0; j < log_num_vals[i]; j++){
                ...
                data_write_size = snprintf(line + offset, sizeof(line) - offset, "| %.2f", log_val[i][j]);
